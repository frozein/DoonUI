...
{
	return {_DN_clamp(v.x, min, max), _DN_clamp(v.y, min, max)};
}
...
DNvec2 DN_vec2_lerp(DNvec2 from, DNvec2 to, GLfloat a)
{
	...
	return {from.x * a2 + to.x * a, from.y * a2 + to.y * a};
} 
...
{
	return {powf(v.x, s), powf(v.y, s), powf(v.z, s)};
}
...
{
	return {_DN_clamp(v.x, min, max), _DN_clamp(v.y, min, max), _DN_clamp(v.z, min, max)};
}
...
DNvec3 DN_vec3_lerp(DNvec3 from, DNvec3 to, GLfloat a)
{
	...
	return {from.x * a2 + to.x * a, from.y * a2 + to.y * a, from.z * a2 + to.z * a};
} 
...
{
	return {powf(v.x, s), powf(v.y, s), powf(v.z, s), powf(v.w, s)};
}
...
{
	return {_DN_clamp(v.x, min, max), _DN_clamp(v.y, min, max), _DN_clamp(v.z, min, max), _DN_clamp(v.w, min, max)};
}
...
DNvec4 DN_vec4_lerp(DNvec4 from, DNvec4 to, GLfloat a)
{
	...
	return {from.x * a2 + to.x * a, from.y * a2 + to.y * a, from.z * a2 + to.z * a, from.w * a2 + to.w * a};
}
